...
```{r VerifyA22}
stopifnot(identical(matR[2,1]^2 + matR[2,2]^2, vecA[2]))
```
...
```{r VerifyR31}
stopifnot(identical(0.5*(matR[1,1] + matR[2,1]), matR[3,1]))
```


$$r_{32} = {1\over 2}(r_{12} + r_{22}) = {1\over 2}(`r matR[1,2]` + `r matR[2,2]`) = `r 0.5*(matR[1,2] + matR[2,2])`$$
```{r VerifyR32}
stopifnot(identical(0.5*(matR[1,2] + matR[2,2]), matR[3,2]))
```
...
```{r VerifyR33}
stopifnot(identical(sqrt(1-0.25*(vecA[1]+vecA[2])), matR[3,3]))
```
...
$$
```{r VerifyA33}
stopifnot(identical(matR[3,1]^2 + matR[3,2]^2 + matR[3,3]^2, vecA[3]))
```
...
```{r VerifyR41}
stopifnot(identical(0.5 * matR[1,1], matR[4,1]))
```

$$r_{42} = {1\over 2} *  r_{12} = {1\over 2}  * `r matR[1,2]` = `r 0.5 * matR[1,2]`$$
```{r VerifyR42}
stopifnot(identical(0.5 * matR[1,2], matR[4,2]))
```

$$r_{43} = {1\over 2} *  r_{13} = {1\over 2}  * `r matR[1,3]` = `r 0.5 * matR[1,3]`$$
```{r VerifyR43}
stopifnot(identical(0.5 * matR[1,3], matR[4,3]))
```

$$r_{44} = \sqrt{{1\over 2}\ - {1\over 4}\ (F_s + F_d)}$$

$a_{ss} = 1 + F_s \rightarrow F_s = a_{ss} - 1$
