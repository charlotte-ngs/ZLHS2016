...
\hat{a}_i &=& \frac{1}{1 + \alpha d^{(i)} + {\alpha\over 4} \sum_{j=1}^n d^{(k_j)}}
              \left[y_i - \hat{\mu} + {\alpha\over 2}\left\{d^{(i)}(\hat{a}_s + \hat{a}_d) 
...
Aus den Mischmodellgleichungen betrachten wir die zweitletzte Gleichung und können so die Formel (\ref{eq:AhatDecompEq}) verifizieren. Dabei gilt zu berücksichtigen, dass $d^{(i)}$ nur drei Werte annehmen kann.
