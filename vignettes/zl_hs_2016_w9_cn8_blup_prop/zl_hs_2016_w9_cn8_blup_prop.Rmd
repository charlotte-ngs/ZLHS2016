...
vecSire <- unique(dfWwgSire$Vater)
vecSire <- vecSire[order(vecSire)]
nNrSire <- length(vecSire)
```

Das Ziel hier ist den fixen Effekt des Geschlechts zu schätzen und Zuchtwerte für die Väter `r vecSire[1:(nNrSire-1)]` und `r vecSire[nNrSire]` vorauszusagen. Die Parameter seien die gleichen wie im Tiermodell, somit ist $\sigma_s^2 = \sigma_a^2/4 = `r sigmas2`$ und $\sigma_e^2 = `r sigmae2sire`$ daraus folgt $\alpha = `r alphasire`$. 
...
#matAInvSire <- as.matrix(getAInv(ped = pedASire))[vecSire,vecSire]
...
cat("\\right]\n")
cat(" = \\left[\n")
cat(paste(sGetTexMatrix(pmatAMatrix = vecRhs, pnDigits = 1, pnAlign = rep("c",2)), collapse = "\n"))
cat("\\right]\n")
cat("$$\n")
```
...
```{r SolutionMmeSire}
vecSol <- solve(matCoeffSire, vecRhs)
cat("$$\\left[\n")
cat(paste(sGetTexMatrix(pmatAMatrix = as.matrix(vecUnknown), pnAlign = rep("c", 2)), collapse = "\n"))
cat("\\right]\n")
cat(" = \\left[\n")
cat(paste(sGetTexMatrix(pmatAMatrix = as.matrix(vecSol), pnDigits = 3, pnAlign = rep("c",2)), collapse = "\n"))
cat("\\right]\n")
cat("$$\n")
```
